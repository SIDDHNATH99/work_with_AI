...
def chat(messages, system=None, tools=None, temperature=1.0, stop_sequences=[]):
    params = {
        "model": model,
        "max_tokens": 1000,
        "messages": messages,
...
def get_current_datetime(date_format="%Y-%m-%d %H:%M:%S"):
    if not date_format:
        return ValueError(f"date format cannot be empty")
...
get_current_datetime_schema = ToolParam({
  ...
  "input_schema": {
    ...
    "properties": {
      "date_format": {
        "type": "string",
        "description": "A strftime-compatible format string that defines the output format. Defaults to '%Y-%m-%d %H:%M:%S' (e.g. '2025-03-19 14:30:00'). Cannot be empty.",
        "default": "%Y-%m-%d %H:%M:%S"
      }
    },
    "required": []
...
def run_conversation(messages):
    ...
    while True:
        ...
        print("text" , text_from_message(response))
...
add_user_message(messages , "set reminder for my doctor's appointment after 150 days from 30th March 2026")
