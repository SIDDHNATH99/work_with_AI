...
export async function fetchCustomerOrders(db, customerId, limit = 10) {
    const query = `
    SELECT 
        o.order_id,
        o.order_date,
...
export async function getPendingOrders(db) {
    const query = `
    SELECT 
        o.id as order_id,
...
export async function findOrdersByStatus(db, status) {
    const query = `
    SELECT DISTINCT
        o.order_id,
        o.order_date,
...
export async function getRecentOrders(db, days = 7) {
    const query = `
    SELECT DISTINCT
        o.order_id,
        o.order_date,
...
export async function fetchOrdersByDateRange(db, startDate, endDate) {
    const query = `
    SELECT 
        o.order_id,
        o.order_date,
...
export async function getHighValueOrders(db, minAmount = 500) {
    const query = `
